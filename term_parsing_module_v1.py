...
class TermElements:
    ...
    operator_priority = 0  # To simulate PEMDAS

    def __init__(self, term, var_input_value):
        self.term = term
        self.var_input_value = float(var_input_value)
        self.post_parse_sub = ElementSubtraction(TermElements(self.term, self.var_input_value).term_parser(self.term))
        self.post_parse_coef = ElementCoefficient(TermElements(self.term, self.var_input_value).term_parser(self.term))
        self.post_parse_var = ElementVariable(TermElements(self.term, self.var_input_value).term_parser(self.term))
        self.post_parse_exp = ElementExponent(TermElements(self.term, self.var_input_value).term_parser(self.term))
...
class ElementSubtraction(TermElements):
    """
    This class is intended to correspond to the "minus_" prefixes present in the elements of subtracted terms
    """

    operator_priority = 2  # To simulate PEMDAS

    def __init__(self, element_list, boolean=None):
        TermElements.__init__(self, element_list)
    ...
    @minus_prefix.setter
    def minus_prefix(self, value=-1.0):
        if self.boolean:
...
class ElementCoefficient(TermElements):

    operator_priority = 3  # To simulate PEMDAS

    def __init__(self, element_list, boolean=None):
        TermElements.__init__(self, element_list)
        self._coefficient_value = None
        self.boolean = boolean  # For checking if the element is present in the term
...
class ElementVariable(TermElements):
...
class ElementExponent(TermElements):

    operator_priority = 4  # To simulate PEMDAS.

    def __init__(self, element_list, notation_boolean=None, value_boolean=None):
        TermElements.__init__(self, element_list)
        self._exponent_value = None
